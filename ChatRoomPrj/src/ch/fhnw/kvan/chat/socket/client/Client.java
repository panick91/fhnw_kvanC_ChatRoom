    ...
    private void handleInput(String key, String value) {
        try {
            ...
            switch (key) {
                ...
                case "messages":
                    log.info("Add messages " + value);
                    String[] messages2 = value.split(";");
                    for (String m : messages2) {
                        if (topics.contains(gui.getCurrentTopic())) {
                            messages.get(gui.getCurrentTopic()).add(m);
                        }
                    }
                    if (messages2.length > 0)
                        gui.updateMessages(messages2);
                    else
                ...
                case "remove_topic":
                    String topic1 = value.substring(1, value.length() - 1);
                    log.info("remove topic " + topic1);
                    messages.remove(topic1);
                    topics.remove(topic1);
                    gui.removeTopic(topic1);
                    break;
            }
        } catch (IOException e) {
            e.printStackTrace();
